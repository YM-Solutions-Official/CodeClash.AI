...
export async function sendOtpValidation(req: Request, res: Response, next: NextFunction) {
  ...
  if (mobile && !mobileRegex.test(mobile)) {
    logger.log({
      level: "info",
      message: JSON.stringify(failureTemplate(400, "Enter Valid Mobile Number")),
    });
    return res
      .status(400)
